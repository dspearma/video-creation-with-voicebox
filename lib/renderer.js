...
/**
 * Trim a video clip to a target duration.
 * Used to cut Flow clips (4/6/8/10s) to the exact time needed.
 *
 ...
 */
function trimClip(inputPath, outputPath, targetDuration) {
...
async function renderProject(scenes, outputDir, onProgress = null) {
  ...
  for (let i = 0; i < scenes.length; i++) {
    ...
    if (mediaType === 'still_image') {
      ...
      // Video scene — may have multiple clips
      const clips = scene.clips || [];
      let sceneVideoPath;

      if (clips.length > 1) {
        // Multi-clip: concatenate all clips, then trim to narration duration
        const clipPaths = clips.map(c => path.join(outputDir, 'video', c.file)).filter(p => fs.existsSync(p));
        if (clipPaths.length === 0) throw new Error(`Missing video clips for scene ${sceneNum}`);

        const rawConcatPath = path.join(outputSubDir, `_raw_scene_${sceneNum}.mp4`);
        await concatenateScenes(clipPaths, rawConcatPath);

        // Trim concatenated clips to narration duration
        sceneVideoPath = path.join(outputSubDir, `_trimmed_scene_${sceneNum}.mp4`);
        await trimClip(rawConcatPath, sceneVideoPath, narrationDuration);
        try { fs.unlinkSync(rawConcatPath); } catch {}
      } else if (clips.length === 1) {
        // Single clip — trim to narration duration
        const clipPath = path.join(outputDir, 'video', clips[0].file);
        if (!fs.existsSync(clipPath)) throw new Error(`Missing video clip for scene ${sceneNum}`);
        sceneVideoPath = path.join(outputSubDir, `_trimmed_scene_${sceneNum}.mp4`);
        await trimClip(clipPath, sceneVideoPath, narrationDuration);
      } else {
        // Legacy single file
        sceneVideoPath = path.join(outputDir, 'video', `scene_${sceneNum}.mp4`);
        if (!fs.existsSync(sceneVideoPath)) throw new Error(`Missing video for scene ${sceneNum}`);
      }

      const result = await mergeSceneWithSfx(sceneVideoPath, audioPath, mergedPath, sfxPath);
      if (result.sfx) {
        const sfxDur = await getMediaDuration(result.sfx);
        sfxSegments.push({ path: result.sfx, startTime: currentTime, duration: sfxDur });
      }
      ...
      if (sceneVideoPath.includes('_trimmed_')) {
        try { fs.unlinkSync(sceneVideoPath); } catch {}
      }
    }
